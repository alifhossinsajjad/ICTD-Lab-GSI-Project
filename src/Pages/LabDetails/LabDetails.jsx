...
const LabDetails = () => {
    ...
    useEffect(() => {
        ...
        fetch("/srd-data300.json")
            ...
            .then((data) => {
                ...
                const dhakaLabs = sanitizedData.filter(lab => lab.division === "Dhaka");
                ...
                (error) => {
                    ...
                    switch (error.code) {
                        case error.PERMISSION_DENIED:
                            errorMessage = "Location permission denied. Please enable location services in your browser and system settings.";
                            break;
                        case error.POSITION_UNAVAILABLE:
                            errorMessage = "Location information is unavailable. Please check your network connection or GPS.";
                            break;
                        case error.TIMEOUT:
    ...
    useEffect(() => {
        ...
        if (currentLocation) {
            ...
            filtered = filtered.filter(lab => lab.lat !== 0 && lab.long !== 0).slice(0, 200);

            // Calculate center of filtered labs for division-based filtering
            if (filtered.length > 0) {
                const validLabs = filtered.filter(lab => lab.lat && lab.long && lab.lat !== 0 && lab.long !== 0);
                if (validLabs.length > 0) {
                    const avgLat = validLabs.reduce((sum, lab) => sum + parseFloat(lab.lat), 0) / validLabs.length;
                    const avgLong = validLabs.reduce((sum, lab) => sum + parseFloat(lab.long), 0) / validLabs.length;

                    // Validate coordinates before setting
                    if (!isNaN(avgLat) && !isNaN(avgLong) && isFinite(avgLat) && isFinite(avgLong)) {
                            ...
                            {selectedLab ? (
                                <div>
                                    ...
                                    <div className="mt-4 pt-4 border-t border-emerald-500/20">
                                        <p className="text-sm font-semibold text-emerald-200/70 mb-3">Lab Images</p>
                    ...
                    >
                        ...
                        <div className="bg-emerald-900 backdrop-blur-xl rounded-2xl shadow-2xl border border-emerald-500/20 p-4 sm:p-6">
                            ...
                            <div className="space-y-2 mb-4">
                                ...
                                >
                                    <option value="All" className="bg-emerald-900 text-white">All Divisions</option>
                        ...
                        {/* Nearby Labs List */}
                        <div className="bg-emerald-900 backdrop-blur-xl rounded-2xl shadow-2xl border border-emerald-500/20 p-4 sm:p-6 max-h-[400px] overflow-y-auto custom-scrollbar">
