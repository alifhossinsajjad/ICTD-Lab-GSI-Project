...
import { FiHome, FiBell, FiInfo, FiTarget, FiCamera, FiUsers } from "react-icons/fi"
...
const Navbar = ({ setActiveView }) => {
  ...
  const handleNavigation = (view) => {
    if (view === "home") {
      ...
      ["notice", "about", "goals", "gallery", "team"].includes(view)
    ) {
      scrollTo(view)
    ...
    <header className="bg-white shadow-lg border-b border-gray-200">
      ...
      <div className="container mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center space-x-1">
          <NavItem icon={<FiHome />} label={t.home} onClick={() => handleNavigation("home")} />
          <NavItem icon={<FiBell />} label={t.notice} onClick={() => handleNavigation("notice")} />
          <NavItem icon={<FiInfo />} label={t.about} onClick={() => handleNavigation("about")} />
          <NavItem icon={<FiTarget />} label={t.goals} onClick={() => handleNavigation("goals")} />
          <NavItem icon={<FiCamera />} label={t.gallery} onClick={() => handleNavigation("gallery")} />
          <NavItem icon={<FiUsers />} label={t.team} onClick={() => handleNavigation("team")} />
          <NavItem icon={<FiBell />} label={t.allNotices} onClick={() => handleNavigation("all-notices")} />
        </nav>
